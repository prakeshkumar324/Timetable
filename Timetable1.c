...
{	
	...
	for(i=0;i<tch;i++)
	{
		printf("enter teacher's name %d ",i);
		scanf("%s",th[i].name);
	}
	for(i=0;i<crs;i++)
	{
		...
		if(ro[tmproom].allsub>lects*dys-dys)
		{
...
void generate(int i,int j,int k)
{
	...
	if((i<crs)&&(set!=crs))
	{
		...
		{
			if((th[co[i].ctchr[x]].busy[j].busy[k].busy==false) && (ro[co[i].room].busy[j].busy[k].busy==false) && (co[i].creadit[x]) && (co[i].allsub<=((dys*lects)-co[i].breaks)))
			{
